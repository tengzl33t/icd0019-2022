...
public class Helpers {
    ...
    private static Card.CardSuit getAvailableRandomSuitFor(Card.CardValue cardValue, Hand hand) {
        Card.CardSuit[] suits = Card.CardSuit.values();

        int randomNumber = new Random().nextInt(suits.length);
        Card.CardSuit randomSuit = suits[randomNumber];
    ...
    public static Hand getHandWithRandomSuit(String cardValues) {
        Map<String, Card.CardValue> map = getCardValueMap();
        ...
        for (String character : cardValues.split("")) {
            Card.CardValue cardValue = map.get(character);
            Card.CardSuit suit = getAvailableRandomSuitFor(cardValue, hand);
            hand.addCard(new Card(cardValue, suit));
        }
        return hand;
    }
